...
def test_real():
    ...
    res["jm1"] = (
        [],
        [
            '"n" AND "v" AND "l" AND "d" AND "i" AND "e" AND "t" AND "lOCode" AND "uniq_Op" AND "uniq_Opnd" AND "total_Op" AND "total_Opnd"'
        ],
    )
    res["nomao"] = (['"V1" AND "V2"', '"V4" AND "V6"'], [])
    res["mc1"] = (
...
def test_synthetic_in_sync_nulls():
    synth_test(test_id, "in-sync", 0, 0)  # Given the null values, there are not sufficient zeros in the columns
