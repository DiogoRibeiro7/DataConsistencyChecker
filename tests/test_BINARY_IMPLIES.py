...
synth_exceptions_cols = ['"bin implies all_1" AND "bin implies most"', '"bin implies all_2" AND "bin implies most"']


@requires_real_data
def test_real():
    res = build_default_results()
    res["isolet"] = (['"f578" AND "f579"', '"f578" AND "f580"', '"f579" AND "f580"'], [])
    res["semeion"] = (0, 16)
    res["scene"] = (
        [
            '"Beach" AND "Sunset"',
    ...
    res["nomao"] = (['"V7" AND "V8"'], [])
    res["PhishingWebsites"] = ([], ['"Favicon" AND "port"', '"on_mouseover" AND "popUpWidnow"'])
    ...
    res["steel-plates-fault"] = (['"V12" AND "V13"', '"V30" AND "V33"'], ['"V12" AND "V30"', '"V13" AND "V30"'])
...
def test_synthetic_one_row_nulls():
    synth_test(test_id, "one-row", 0, 3)  # The pattern is now an exception
